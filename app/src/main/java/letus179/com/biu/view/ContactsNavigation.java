...
public class ContactsNavigation extends View {
    ...
    //手指按下的字母改变接口
    private onContactsChangeListener listener;

    public ContactsNavigation(Context context) {
        this(context, null);
    }

    public ContactsNavigation(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init();
    }
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (listener != null && 0 <= touchIndex && touchIndex <= words.length - 1) {
                    //回调按下的字母
                    listener.contactsChange(words[touchIndex]);
                }
                invalidate();
                break;
    ...
    public interface onContactsChangeListener {
        void contactsChange(String words);
    }
    /**
     *设置手指按下字母改变监听
     */
